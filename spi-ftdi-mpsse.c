...
// FTDI_MPSSE_GPIOS = mpsse_get_numgpio(intf);

struct ftdi_spi {
	struct platform_device *pdev;
	struct usb_interface *intf;
	struct spi_controller *master;
	const struct mpsse_ops *iops;
...
{
	const struct mpsse_ops *ops = priv->iops;
...
//	int ret2;
	...
	model = mpsse_get_model(priv->intf);
	priv->ftmodel = model;
	dev_info(dev, "model num %d\n", priv->ftmodel);

	numgpio = mpsse_get_numgpio(priv->intf);
